...
import (
	...
	"github.com/crossplane-contrib/crossplane-provider-newrelic/internal/features"
	"github.com/crossplane-contrib/crossplane-provider-newrelic/internal/version"
)
...
func main() {
	var (
		...
		pollInterval            = app.Flag("poll", "Poll interval controls how often an individual resource should be checked for drift.").Default("1h").Duration()
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped NewRelic APIs to scheme")
	...
	if *enableChangeLogs {
		...
		conn, connErr := grpc.NewClient("unix://"+*changelogsSocketPath, grpc.WithTransportCredentials(insecure.NewCredentials()))
		kingpin.FatalIfError(connErr, "failed to create change logs client connection at %s", *changelogsSocketPath)
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped NewRelic controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced NewRelic controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped NewRelic controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced NewRelic controllers")
	}
