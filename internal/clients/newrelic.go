...
import (
	...
	clusterv1beta1 "github.com/crossplane-contrib/crossplane-provider-newrelic/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/crossplane-contrib/crossplane-provider-newrelic/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		cfgKeys := []string{"account_id", "api_key", "region", "insecure_skip_verify", "insights_insert_key", "cacert_file"}
		for _, cfgKey := range cfgKeys {
			if v, ok := creds[cfgKey]; ok {
				ps.Configuration[cfgKey] = v
			}
		}
		return ps, nil
	}
...
func resolveProviderConfig(ctx context.Context, crClient client.Client, mg resource.Managed) (*namespacedv1beta1.ProviderConfigSpec, error) {
	...
	case resource.LegacyManaged: //nolint:staticcheck // Required for backward compatibility with cluster-scoped resources
...
//nolint:staticcheck // Required for backward compatibility with cluster-scoped resources
